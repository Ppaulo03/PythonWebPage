...
class S {
  ...
  constructor({
    ...
    for (this.x = t, this.y = i, this.ctx = s, this.hue = h, this.gravity = r, this.friction = c, this.flickering = u, this.lineWidth = p, this.explosionLength = f, this.angle = l(0, Math.PI * 2), this.speed = o(1, 10), this.brightness = o(d.min, d.max), this.decay = l(a.min, a.max); this.explosionLength--; )
  ...
  draw() {
    ...
    this.ctx.beginPath(), this.ctx.lineWidth = this.lineWidth, this.ctx.fillStyle = g(this.hue, this.brightness, this.alpha), this.ctx.moveTo(
      ...
      this.hue,
      this.flickering ? l(0, this.brightness) : this.brightness,
      this.alpha
    ), this.ctx.stroke();
  }
}
...
class L {
  ...
  pointerDown(t) {
    this.usePointer(t, n.mouse.click);
  }
  pointerUp(t) {
    this.usePointer(t, !1);
  }
...
class M {
  ...
  start() {
    ...
    const t = 1e3 / this.fps, i = (s) => {
      ...
      const h = s - this.now;
      h >= t - this.tolerance && (this.render(), this.now = s - h % t, this.tick += h * (n.intensity * Math.PI) / 1e3);
    };
    this.rafId = requestAnimationFrame(i);
  }
  stop() {
...
class z {
  ...
  subscribe() {
    n.autoresize && window.addEventListener("resize", this.handleResize);
  }
  unsubscribe() {
    n.autoresize && window.removeEventListener("resize", this.handleResize);
  }
  handleResize() {
...
class C {
  ...
  play() {
    if (n.sound.enabled && this.sounds.length) {
      const t = this.audioContext.createBufferSource(), i = this.sounds[o(0, this.sounds.length - 1)], s = this.audioContext.createGain();
      t.buffer = i, s.gain.value = l(
        n.sound.volume.min / 100,
        n.sound.volume.max / 100
...
class O {
  ...
  constructor({
    ...
    for (this.x = t, this.y = i, this.sx = t, this.sy = i, this.dx = s, this.dy = h, this.ctx = a, this.hue = r, this.speed = c, this.traceLength = d, this.acceleration = u, this.totalDistance = m(t, i, s, h), this.angle = Math.atan2(h - i, s - t), this.brightness = o(50, 70); this.traceLength--; )
      this.coordinates.push([t, i]);
  }
  update(t) {
...
class R {
  ...
  render() {
    if (!this.ctx || !this.running)
      return;
    const { opacity: t, lineStyle: i, lineWidth: s } = n;
  ...
  createTrace() {
    ...
    this.traces.push(
      new O({
        ...
        y: this.height,
        dx: this.mouse.x && c.move || this.mouse.active ? this.mouse.x : o(s.x, s.width - s.x * 2),
        dy: this.mouse.y && c.move || this.mouse.active ? this.mouse.y : o(s.y, s.height * 0.5),
        ctx: this.ctx,
        hue: o(t.min, t.max),
        speed: a,
        acceleration: r,
  ...
  initTrace() {
    this.waitStopRaf || (this.raf.tick > o(n.delay.min, n.delay.max) || this.mouse.active && n.mouse.max > this.traces.length) && (this.createTrace(), this.raf.tick = 0);
  }
  drawTrace() {
